...
/*   Updated: 2023/11/17 11:36:14 by alimotta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

char	*fill_static_array(char **temp, char **s, int fz)
{
	int		j;
	size_t	len;
	size_t	len_temp;
...
int	fill_temp_array(char **temp, char **s, int fz)
{
	char	*temp2;
	size_t	len;
	size_t	len_src;
...
char	*emp_sta(char **static_arr, char **temp, size_t len)
{
	*temp = (char *)malloc((len + 1));
...
char	*get_next_line(int fd)
{
	...
	temp = 0;
	if (static_arr[fd] != 0)
		temp = emp_sta(&static_arr[fd], &temp, ft_strlen(static_arr[fd]));
	while (check == -1)
	{
		fz = fill_s(s, fd);
		if (fz < 0)
	...
	static_arr[fd] = fill_static_array(&temp, &s, fz);
